...
<script type="text/javascript">
	$(function() {
		...
		wordgrid = $('#wordgrid').datagrid({
			...
			toolbar : [ /*{
				...
				handler : function() {
    				var filename = encodeURI(encodeURI("resources/template/文言文字词导入模板.xls"));
					...
					if (result.success) {
						...
						$.each(result.obj.list,function(index,element) {
							...
							for(var i = 0; i < yxs.length; i++) {
								...
								yxdiv += '<div id="additional' + yxs[i].ID + '"><label>义项:</label><span name="explain">' + yxs[i].MEANING + '</span><br/><div id="examples' + yxs[i].ID + '">';
								if(yxs[i].EXAMPLES) {
	...
	function updateBasic() {
		$('#updBasicDialog').dialog({
			...
			onOpen : function() {
				...
				var selectedData = $('#wordgrid').datagrid('getSelected');
				$('#basicForm').form('load','getClassicalWordBasic?id=' + selectedData.id);
			}
		});
	}
	...
	function updateAdditional(id) {
		$('#updAdditionalDialog').dialog({
			...
			buttons : [ {
				...
				handler : function() {
					...
					$('#additionalForm').form('submit', {
						url : 'updateAdditionalClassicalWord',
						onSubmit : function() {
			...
			onOpen : function() {
				...
				$('#additionalForm').form('load','getClassicalWordAdditional?id=' + id);
			}
		});
	}
			...
			buttons : [ {
				...
				handler : function() {
					...
					$('#exampleForm').form('submit', {
						url : 'addExampleClassicalWord?id=' + id,
						onSubmit : function() {
	...
	function updExample(id) {
		$('#updExampleDialog').dialog({
			...
			onOpen : function() {
				$('#updExampleForm').form('load','getClassicalExampleById?id=' + id);
			}
		});
	}
		...
		<form id="addForm" method="post">
			<table>
				...
				<tr>
					<td>义项</td>
					<td>
						<input type="text" name="meaning" class="easyui-validatebox"/>
					</td>
				</tr>
				<tr>
					<td>例句</td>
					<td>
						<textarea rows="2" cols="21" name="example" data-options="validType:'length[1,5000]'"></textarea>
					</td>
				</tr>
			</table>
		</form>
		...
		<form id="additionalForm" method="post">
			<table>
				...
				<tr>
					<td>义项/用法详细</td>
					<td>
						<input type="text" name="meaningInfo" data-options="required:true,validType:'length[1,1000]'"></textarea>
					</td>
				</tr>
			</table>
		</form>
